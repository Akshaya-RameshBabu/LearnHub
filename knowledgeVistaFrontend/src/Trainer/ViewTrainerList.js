  ...
  const searchUsers = async () => {
    try {
      const response = await axios.get(
        `${baseUrl}/Institution/search/Trainer`,
        {
          headers: {
            Authorization: token,
          },
        ...
        try {
          if (userId != null) {
            const response = await axios.delete(
              `${baseUrl}/admin/deactivate/trainer`,
              {
                data: formData,
                headers: {
                  Authorization: token,
    ...
    MySwal.fire({
      ...
      if (result.isConfirmed) {
        try {
          if (userId != null) {
            const response = await axios.delete(
              `${baseUrl}/admin/Activate/trainer`,
              {
                data: formData,
                headers: {
                  Authorization: token,
                    ...
                    {filterData().map((user, index) => (
                      <tr key={user.userId}>
                        ...
                        {userRole === "ADMIN" ? (
                          <td className="text-center">
                            <Link
                              to={`/assignCourse/Trainer/${user.userId}`}
                              className="hidebtn"
                            >
