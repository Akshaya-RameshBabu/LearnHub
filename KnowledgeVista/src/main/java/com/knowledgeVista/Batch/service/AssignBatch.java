...
public class AssignBatch {
	...
	public ResponseEntity<?> assignCoursesToUser(HttpServletRequest request, Long userId, Long BatchId, String token) {
		try {
			...
			if (opbatch.isPresent()) {
				...
				user.getCourses().addAll(courses.stream().filter(course -> !user.getCourses().contains(course)) // Filter
																												// out
																												// existing
																												// courses
	...
	public void sendEnrollmentMail(HttpServletRequest request, List<CourseDetail> courses, Batch batch, Muser student,
		...
		if ("TRAINER".equals(type)) {
			body.append("<html>").append("<body>").append("<h2>Welcome to LearnHub - Your Trainer Journey Begins!</h2>")
					...
					.append(batch.getBatchTitle()).append(" (").append(formattedStartDate).append(" to ")
					.append(formattedEndDate).append(")</strong> at LearnHub.</p>")
					...
					.append("<h3>Your Responsibilities:</h3>").append("<ul>")
					...
					.append("<li>Track student progress and provide constructive feedback.</li>").append("</ul>")

					.append("<h3>Courses You Will Handle:</h3>").append("<ul style='font-weight: bold;'>")
					.append(generateCourseList(batch.getCourses())).append("</ul>")

					.append("<h3>Getting Started:</h3>").append("<ul>")
					...
					.append("<h3>Exclusive Trainer Benefits:</h3>").append("<ul>")
					...
					.append("<li>Support from our dedicated trainer community and technical team.</li>").append("</ul>")
					...
					.append("<p>Click the link below to sign in and begin your journey:</p>").append("<p><a href='")
					.append(signInLink)
					.append("' style='font-size:16px; color:blue;'>Sign In to Your Trainer Dashboard</a></p>")

					.append("<p>Wishing you a successful and fulfilling training experience!</p>")
					.append("<p>Best Regards,<br><strong>LearnHub Team</strong></p>").append("</body>")
					.append("</html>");

		} else {

			body.append("<html>").append("<body>")
					.append("<h2>Welcome to LearnHub - Your Learning Journey Begins!</h2>").append("<p>Dear ")
					.append(student.getUsername()).append(",</p>")
					...
					.append("<ul style='font-weight: bold;'>").append(generateCourseList(batch.getCourses()))
					.append("</ul>").append("<p>To get started:</p>").append("<ul>")
					.append("<li>Log in to your LearnHub account.</li>")
					.append("<li>Access your enrolled courses in My Courses Tab.</li>")
					.append("<li>Engage with trainers and fellow students.</li>")
					.append("<li>Complete assignments and track your progress.</li>").append("</ul>")
					.append("<p>If you need any assistance, our support team is here to help.</p>")
					.append("<p>Click the link below to sign in:</p>").append("<p><a href='").append(signInLink)
					.append("' style='font-size:16px; color:blue;'>Sign In</a></p>")
		...
		if (institutionname != null && !institutionname.isEmpty()) {
			try {
				...
				emailService.sendHtmlEmailAsync(institutionname, emailList, cc, bcc,
						"Welcome to LearnHub - Batch Enrollment Successful!", body.toString());
			} catch (Exception e) {
...
////============================Assign Batch To Trainer====================================
	@Transactional
	public ResponseEntity<?> assignBatchesToTrainer(HttpServletRequest request, Long userId, Long BatchId,
			String token) {
		try {
			// ✅ Fetch admin user details once
			Optional<Muser> opUser = muserRepository.findByEmail(jwtUtil.getEmailFromToken(token));
			...
			if (opbatch.isPresent()) {
				...
				user.getAllotedCourses()
						.addAll(courses.stream().filter(course -> !user.getAllotedCourses().contains(course)) // Filter
																												// out
																												// existing
																												// courses
								.toList() // Collect remaining courses into a list
	...
	private void createAndSendNotification(Muser user, Batch batch, List<Long> trainerList, String addingEmail,
			String institution, boolean isTrainer) {
		try {
			...
			Long notifyId = notiservice.createNotification("CourseAssigned", user.getUsername(), notiDescription,
					addingEmail, heading, link, batch.getBatchImage());
			if (notifyId != null) {
			...
			if (!isTrainer && trainerList != null && !trainerList.isEmpty()) {
				...
				String notiDescriptionT = "A New Student " + user.getUsername() + " was assigned to your batch "
						+ batch.getBatchTitle();

				Long notifyIdT = notiservice.createNotification("StudentAssigned", user.getUsername(), notiDescriptionT,
						addingEmail, headingT, linkT, user.getProfile());
				if (notifyIdT != null) {
			...
			String linkA = isTrainer ? "/assignCourse/Trainer/" + user.getUserId()
					: "/assignCourse/Student/" + user.getUserId();
			String notiDescriptionA = "A batch " + batch.getBatchTitle() + " was assigned to " + user.getUsername();

			Long notifyIdA = notiservice.createNotification("StudentAssigned", user.getUsername(), notiDescriptionA,
					addingEmail, headingA, linkA, user.getProfile());
			if (notifyIdA != null) {
